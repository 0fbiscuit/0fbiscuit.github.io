---
title: Boot2Root - Machine 0x546f6d6257617463686572
published: 2025-08-04
description: Bài viết này là quá trình mình khai thác Machine 0x546f6d6257617463686572, một bài lab thực tế mô phỏng môi trường Active Directory với nhiều kỹ thuật leo thang đặc trưng.
tags: [redteam, windows, real-life, pentest, medium]
category: Boot2Root
draft: false
---

# Machine 0x546f6d6257617463686572 - H
...
Sau khi có được list users của domain `tombwatcher.htb` thì mình dùng **bloodhound** để bắt đầu truy vết mối quan hệ giữa các users rồi mò đường để khai thác:
...
Okay, đây là sơ đồ biểu diễn mối quan hệ từ **Henry** đến **John:**

- **Bắt đầu từ Henry:**
    - **Henry** có quyền `WriteSPN` lên user `ALFRED@TOMBWATCHER.HTB`.
- **Vậy thì WriteSPN là gì?**
    - Khi một user có quyền `WriteSPN` lên một tài khoản khác, ta có thể thêm một SPN (Service Principal Name) cho tài khoản đó, rồi thực hiện Targeted Kerberoasting → lấy hash và crack hash để lấy được password.
    - Đây là kĩ thuật thường dùng để leo thang từ user này sang user khác.
- **Alfred → INFRASTRUCTURE group:**
    - **Alfred** có thể `AddSelf` vào nhóm `INFRASTRUCTURE@TOMBWATCHER.HTB` .
- **INFRASTRUCTURE group → ANSIBLE_DEV$:**
    - Nhóm `INFRASTRUCTURE` có quyền `ReadGMSAPassword` từ `ANSIBLE_DEV$`.
- **ANSIBLE_DEV$ → Sam:**
    - `ANSIBLE_DEV$` có thể `ForceChangePassword` lên user `SAM`.
- **Sam → John:**
    - `SAM` có quyền `WriteOwner` lên `JOHN`.

⇒ **John** có quyền truy cập từ xa (Remote Desktop Users)

## User flag:

---

Dưới đây là từng bước để lấy được user flag:

- **Henry → Alfred:** **WriteSPN** → dùng **Kerberoasting** để lấy hash của Alfred.
- **Alfred → AddSelf** vào nhóm **INFRASTRUCTURE**.
- **INFRASTRUCTURE → ReadGMSAPassword → ANSIBLE_DEV$**
- **ANSIBLE_DEV$ → ForceChangePassword → Sam**
- **Sam→ WriteOwner → John→** truy cập từ xa.
...
Lợi dụng quyền ghi **SPN** để tạo vé **Kerberos** và lấy hash của user **Alfred:**
...
### **2. Alfred → AddSelf** vào nhóm **INFRASTRUCTURE**.

Thêm user **Alfred** vào nhóm **INFRASTRUCTURE:**
...
Khai thác quyền **ReadGMSAPassword** để chiếm tài khoản dịch vụ:

- Ta biết được là `INFRASTRUCTURE` là một **nhóm người dùng** có quyền `ReadGMSAPassword` đối với account `ANSIBLE_DEV$` (một **gMSA** - group Managed Service Account). **gMSA** được thiết kế để các **computer accounts** hoặc group được ủy quyền có thể lấy **plaintext password** từ **DC**.  ****Mà user `alfred` là thành viên của nhóm `INFRASTRUCTURE`, nên có quyền đọc được mật khẩu của **gMSA** `ANSIBLE_DEV$` .

Trích xuất mật khẩu của **gMSA** từ **AD** bằng tài khoản của user **Alfred**:
...
Khai thác quyền **ForceChangePassword** để chiếm quyền người dùng:

- Quyền `ForceChangePassword` cho phép một tài khoản attacker đặt lại mật khẩu của tài khoản user mà **không cần biết mật khẩu cũ**. Từ đó có thể đổi thành bất kì mật khẩu nào để truy cập được vào tài khoản đó, ở đây là tài khoản của user `sam` .
- Tài khoản `ANSIBLE_DEV$` có quyền `ForceChangePassword` đối với người dùng `sam` nên sẽ tấn công để đổi mật khẩu của user này được.

Thay đổi mật khẩu của user **SAM:**
...
### **5. Sam→ WriteOwner → John→** Truy cập từ xa

Sử dụng quyền **WriteOwner** để chiếm quyền user **JOHN**:

- Tài khoản `sam` có quyền **WriteOwner** trên user `john`, tức là có thể thay đổi chủ sở hữu object AD của `john`.
- Một khi `sam` trở thành owner của `john`, thì `sam` có thể tự cấp thêm quyền **FullControl** (nhờ thuộc tính kế thừa của "owner").

Thay đổi **owner** của user `john` sang `sam`
...
Tuy nhiên, khi thay đổi **owner**, chúng ta chỉ thay đổi chủ sở hữu của object `john` , lúc này `sam` - chủ sở hữu mới chưa có **FullControll**, nên mình phải cấp quyền cho user `sam` :
...
Lúc này chúng ta đã có quyền đổi mật khẩu tài khoản `john` sau khi có quyền điều khiển thông qua **GenericAll**:
...
Đăng nhập **WinRM** bằng tài khoản của `john` là đọc flag thôi!!!
...
Thực ra lúc đọc output mình hoảng luôn, hơn 700 dòng…, trông thì cũng không có gì để khai thác, nhưng mà nhìn kĩ thì cũng ta có thể thấy rằng có thể khai thác **ADCS Vulnerabilities - ESC15**:
...
- Có một  **SID** bị “mồ côi” (orphaned) xuất hiện trong output của template: 
`[!] Failed to lookup object with SID 'S-1-5-21-1392491010-1358638721-2126982587-1111'` , vậy **SID** này là của user nào?

Giả định trong template này, điều kiện để  một template dễ bị khai thác **ESC15** sẽ như sau:
...
- Vậy ta thấy, có ít nhất 4 điều kiện thỏa mãn để chúng ta khai thác **ESC15** rồi:

Cuối cùng thì nhìn vào **bloodhound**, ta lại không thấy `cert_admin` , vậy là user này đã bị xóa?, vậy **SID** ởtrong output của template là của user này?

<img width="989" height="613" alt="image 1" src="https://github.com/user-attachments/assets/7cd11e18-03e8-49d6-98b6-a92e41bc9e32" />

Bây giờ thì thử kiểm tra trong thùng rác của **Active Directory** để tìm lại user này xem có đúng là user này đã bị xóa không:
...
- Đối tượng bị xóa đang tìm: `CN=cert_admin\0ADEL:938182c3-bf0b-410a-9aaa-45c8e1a02ebf,CN=Deleted Objects,DC=tombwatcher,DC=htb`
- Vì đối tượng này có SID: `S-1-5-21-1392491010-1358638721-2126982587-1111`

Okay, tới đây thì có thể nói là khả năng vì `cert_admin` bị xóa, **SID** bị orphaned nên là nó không resolve thành user/group, dẫn tới Certipy không đánh dấu rõ **ESC15**.

Bước tiếp theo ta sẽ khôi phục và reset mật khẩu cho user này:
...
Sau đó kiểm tra lại 1 lần nữa xem Certipy có quét được **ESC15** không cho chắc ăn:
...
Vậy là chúng ta khẳng định rằng có thể exploit bằng [**ESC15**](https://github.com/ly4k/Certipy/wiki/06-%E2%80%90-Privilege-Escalation#esc15-arbitrary-application-policy-injection-in-v1-templates-cve-2024-49019-ekuwu).

### ESC15 Vulnerability

---

Trong quá trình khai thác lỗ hổng **ESC15**, chúng ta không thể trực tiếp yêu cầu một chứng chỉ dưới danh tính của **Domain Admin** vì **CA** sẽ từ chối yêu cầu nếu như **không có quyền** phù hợp. Để vượt qua giới hạn đó, chúng ta cần trước tiên yêu cầu một chứng chỉ với **Application Policy** là chứng chỉ “**Request Agent”**.

Chứng chỉ này đóng vai trò như một “ủy quyền” đặc biệt, cho phép tài khoản `cert_admin` thực hiện hành vi **yêu cầu chứng chỉ thay mặt người dùng khác** (như Administrator). Khi đã có chứng chỉ **Request Agent**, `cert_admin` có thể gửi yêu cầu đến CA để cấp một chứng chỉ cho **Domain Admin** bằng cách sử dụng tham số `-on-behalf-of`. Đây là phần quan trọng trong kĩ thuật **giả mạo chứng chỉ để lấy quyền Kerberos TGT**, và là điều kiện tiên quyết để thực hiện thành công khai thác ESC15.

---

Dùng `cert_admin` yêu cầu một **chứng chỉ Request Agent:**
...
Access và lấy root flag thôi:>

<img width="688" height="189" alt="image 2" src="https://github.com/user-attachments/assets/eb42cf39-9a07-4151-8791-347b375910f3" />
